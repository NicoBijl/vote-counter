...
describe("BallotPosition", () => {
  ...
  const mockProps: BallotPositionProps = {
    ...
    setChecked: vi.fn(),
    maxReached: vi.fn().mockReturnValue(false),
  };
  ...
  it("calls setChecked when checkbox is clicked", () => {
    ...
    expect(mockProps.setChecked).toHaveBeenCalledWith(
      "test-position",
      "person1",
      true,
    );
  ...
  it("disables checkboxes when max is reached", () => {
    const maxReachedProps: BallotPositionProps = {
      ...mockProps,
      maxReached: vi.fn().mockReturnValue(true),
    };
  ...
  it("calls setFocusPosition when focused", () => {
    render(<BallotPosition {...mockProps} />);
    const positionElement = screen
      .getByText("Test Position")
      .closest("[tabindex]");
  ...
  it("disables person checkboxes when invalid is selected", () => {
    const invalidSelectedProps: BallotPositionProps = {
      ...
      isChecked: vi
        .fn()
        .mockImplementation((_, person) => person === "invalid"),
    };
