...
describe("Results", () => {
  ...
  it("displays persons in original order when sortResultsByVoteCount is false", () => {
    ...
    const personItems = listItems.filter(
      (item) =>
  ...
  it("correctly identifies candidate status for candidates above divisor", () => {
    ...
    expect(
      screen.getByTestId("person-chip-sorted-above-test-position-person1"),
    ).toBeInTheDocument();

    // Person 2 and 3 should be ABOVE_DIVISOR (has more votes than divisor but not in top candidates)
    expect(
      screen.getByTestId("person-chip-sorted-above-test-position-person2"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("person-chip-sorted-above-test-position-person3"),
    ).toBeInTheDocument();

    // Person 4 should be BELOW_DIVISOR (has fewer votes than divisor)
    expect(
      screen.getByTestId("person-chip-sorted-below-test-position-person4"),
    ).toBeInTheDocument();
  });
  ...
  it("correctly identifies candidate status for candidates below divisor", () => {
    ...
    expect(
      screen.getByTestId("person-chip-sorted-below-test-position-person1"),
    ).toBeInTheDocument();
