...
const mockSettingsStore = {
  ...
  setElectoralDivisorVariable: vi.fn(),
  totalAllowedVoters: 100,
  setTotalAllowedVoters: vi.fn(),
  sortResultsByVoteCount: true,
  setSortResultsByVoteCount: vi.fn(),
};

// Mock the hooks with store creation
vi.mock("../../../hooks/usePositionsStore", () => ({
  usePositionsStore: vi.fn((selector) => {
...
vi.mock("../../../hooks/useBallotStore", () => ({
  useBallotStore: vi.fn((selector) => {
    if (typeof selector === "function") {
...
describe("Settings", () => {
  beforeEach(() => {
    ...
    if (global.URL.createObjectURL === undefined) {
      global.URL.createObjectURL = vi
        .fn()
        .mockReturnValue("mock-url") as unknown as (
  ...
  it("handles position import with valid file", async () => {
    ...
    const file = new File([JSON.stringify(validPositions)], "positions.json", {
      type: "application/json",
    });

    render(<Settings />);
    const fileInput = document.querySelector(
      'input[id="importPositions"]',
    ) as HTMLInputElement;
    fireEvent.change(fileInput, { target: { files: [file] } });

    await waitFor(() => {
      expect(mockPositionsStore.setPositions).toHaveBeenCalledWith(
        validPositions,
      );
      expect(
        screen.getByText(/Positions successfully imported/),
      ).toBeInTheDocument();
    });
  });
  ...
  it("handles legacy position format conversion", async () => {
    ...
    const file = new File([JSON.stringify(legacyPositions)], "legacy.json", {
      type: "application/json",
    });
  ...
  it("handles vote import with valid file", async () => {
    ...
    const file = new File([JSON.stringify(validBallots)], "ballots.json", {
      type: "application/json",
    });

    render(<Settings />);
    const fileInput = document.querySelector(
      'input[id="importVotes"]',
    ) as HTMLInputElement;
    fireEvent.change(fileInput, { target: { files: [file] } });
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/Votes successfully imported/),
      ).toBeInTheDocument();
    });
  });
  ...
  it("handles vote import with invalid format", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          "Invalid ballot format. Please check the file structure.",
        ),
      ).toBeInTheDocument();
    });
  });
  ...
  it("updates electoral divisor", () => {
    ...
    expect(mockSettingsStore.setElectoralDivisorVariable).toHaveBeenCalledWith(
      0.9,
    );
  });
  ...
  it("exports positions when clicked", () => {
    ...
    expect(mockLink.href).toContain(
      encodeURIComponent(JSON.stringify(testPositions)),
    );
  ...
  it("exports votes when clicked", () => {
    ...
    expect(mockLink.href).toContain(
      encodeURIComponent(JSON.stringify(testBallots)),
    );
